#!/bin/sh
# End-to-end priority resolution check against a live bridge:
# two sessions racing through states; the bridge log must show the
# dominant display state at each step. Exits non-zero on failure.
cd "$(dirname "$0")/.." || exit 1

LOG=$(mktemp) || exit 1
# short TTL so the expiry step runs in seconds, not minutes
PET_BRIDGE_SESSION_TTL_MS=1200 node bridge.js > "$LOG" 2>&1 &
BRIDGE_PID=$!
...
# session TTL: a's idle expires, nothing else alive → idle broadcast
sleep 1.6
post idle b
...
check 'state: idle'
rm -f "$LOG"
echo 'PASS: priority resolution end-to-end'
