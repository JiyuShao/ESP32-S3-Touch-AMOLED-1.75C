...
8. 作为开发者,我想列表页滑动支持从任意起点起手(包括屏幕边缘),以便右滑返回的自然手势(从左边缘起手)可用;当未来构建开启手势识别、Phone Shell 边缘导航恢复生效时,再恢复边缘区域豁免。
...
- 手势:全屏(含边缘)手写水平滑动判定(按下→抬起,水平位移阈值 ~60px 且 |dx|>|dy| 水平主导);不豁免边缘区域——当前构建 LV_USE_GESTURE_RECOGNITION=0,Phone Shell 的边缘导航手势同样失效(上板实测:边缘起手滑动无任何 shell 反应),豁免只会吞掉"从左边缘右滑返回"的自然手势;未来若开启手势识别需恢复边缘豁免。
